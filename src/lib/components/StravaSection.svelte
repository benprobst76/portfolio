  ...
  function fallbackToManualEmbeds(placeholders: NodeListOf<Element>) {
    ...
    placeholders.forEach((placeholder) => {
      ...
      if (embedId && embedId !== '0') {
        ...
        iframe.src = `https://www.strava.com/activities/${embedId}/embed/dark`;
        iframe.width = '100%';
        iframe.height = '405';
...
<div class="text-center mb-16">
  <h2 class="text-4xl font-bold mb-4">Some Critical Performance Metrics</h2>
  <p class="text-xl max-w-3xl mx-auto">
    Fitness metrics and activity logs...?
  </p>
</div>
...
{#if loading}
  <div class="text-center">
    ...
    <p class="mt-4 !text-muted-foreground">Loading Strava data...</p>
  </div>
{:else if error}
  <div class="text-center">
...
{:else if stravaData}
  <!-- Activity Breakdown and Athlete Profile Section -->
              ...
              <div class="flex flex-wrap justify-center gap-3">
                {#each activityPercentages as { type, percent }}
                  <div class="flex items-center">
                    ...
                    <span class="text-sm text-muted-foreground">{type} ({percent.toFixed(0)}%)</span>
                  </div>
                {/each}
              </div>
            </div>
        ...
        <div class="flex items-center justify-center">
          <div class="flex items-center gap-6 mb-10">
            ...
            <a 
              ...
              class="inline-flex items-center px-3 py-1 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors text-sm"
            >
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="text-center p-3 bg-gray-50 rounded-lg">
            <div class="text-2xl font-bold text-red-500 mb-1">
              {stravaData.stats.all_run_totals.distance.toLocaleString()}
            </div>
            <div class="text-sm text-muted-foreground font-medium">KM Distance 🗺️</div>
          </div>
          <div class="text-center p-3 bg-gray-50 rounded-lg">
            <div class="text-2xl font-bold text-red-500 mb-1">
              {stravaData.stats.all_run_totals.count.toLocaleString()}
            </div>
            <div class="text-sm text-muted-foreground font-medium">Total Runs 🏃‍♂️</div>
          </div>
          <div class="text-center p-3 bg-gray-50 rounded-lg">
            <div class="text-2xl font-bold text-red-500 mb-1">
              {formatTime(stravaData.stats.all_run_totals.moving_time)}
            </div>
            <div class="text-sm text-muted-foreground font-medium">Moving Time ⏱️</div>
          </div>
          <div class="text-center p-3 bg-gray-50 rounded-lg">
            <div class="text-2xl font-bold text-red-500 mb-1">
              {stravaData.stats.all_run_totals.elevation_gain.toLocaleString()}m
            </div>
            <div class="text-sm text-muted-foreground font-medium">Elevation Gain 🏔️</div>
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  <div class="max-w-4xl mx-auto">
    <h3 class="text-2xl font-bold !text-secondary mb-6 text-center">Recent Activities</h3>
        ...
        <div class="relative">
          <div class="strava-embed-placeholder w-full bg-gray-100 rounded-lg border border-gray-300 overflow-hidden" 
            ...
            <div class="text-center p-6 flex flex-col justify-center h-full min-h-[405px]">
              ...
              <p class="text-sm !text-muted-foreground mb-4">{formatDate(activity.start_date)}</p>
